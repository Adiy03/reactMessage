...
router.get('/history', [passport.authenticate('user', { session: false })],BookingController.getHistory)
//router.put('/update', [passport.authenticate('user', { session: false })],BookingController.update)
//router.get('/:id_field/booked', bookingValidator.getBookedTimeslot, BookingController.getBookedTimeslotPersonal)
router.post('/:id_field/bookedfield',[passport.authenticate('user', { session: false }), bookingValidator.getBookedTimeslot], BookingController.getBookedTimeslotField) // If accessing localhost:3000/transaksi, it will call getAll function in TransaksiController class
...
router.delete('/delete/:id',[passport.authenticate('user', { session: false })], BookingController.delete) // If accessing localhost:3000/transaksi/delete/:id, it will call delete function in TransaksiController class
