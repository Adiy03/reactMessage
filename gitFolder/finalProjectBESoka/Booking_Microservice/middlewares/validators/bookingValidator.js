...
module.exports = {
  ...
  getBookedTimeslot: [
    check('date', 'please, input the date!').exists(),
  ...
  BookedAsField: [
    ...
    check('date').custom(async (value, {
      ...
      for (let i = 0; i < splitted.length; i++) {
        var data2 = await timeslot.findOne({
          where: {
            id: splitted[i]
          }
        });

        coba2.push(data2.dataValues.timeslot)
      }
      ...
      var filterbooked = fieldbooked.filter(function (slot) {
        for (let i = 0; i < 5; i++) {
          for (let j = 0; j < 5; j++) {
            if (coba2[i] == slot.timeslot[j]) {
              return slot.timeslot[j];
            }
          }
        }
